...
func NewReplica(id int, peerAddrList []string, thrifty bool, exec bool, dreply bool, beacon bool, durable bool) *Replica {
	...
	r.preAcceptRPC = r.RegisterRPC(new(epaxosprotobuf.PreAccept), r.preAcceptChan)
	r.preAcceptReplyRPC = r.RegisterRPC(new(epaxosprotobuf.PreAcceptReply), r.preAcceptReplyChan)
	r.preAcceptOKRPC = r.RegisterRPC(new(epaxosprotobuf.PreAcceptOK), r.preAcceptOKChan)
	r.acceptRPC = r.RegisterRPC(new(epaxosprotobuf.Accept), r.acceptChan)
	r.acceptReplyRPC = r.RegisterRPC(new(epaxosprotobuf.AcceptReply), r.acceptReplyChan)
	r.commitRPC = r.RegisterRPC(new(epaxosprotobuf.Commit), r.commitChan)
	r.commitShortRPC = r.RegisterRPC(new(epaxosprotobuf.CommitShort), r.commitShortChan)
...
func (r *Replica) run() {
	...
	for !r.Shutdown {
		...
		case preAcceptS := <-r.preAcceptChan:
			preAccept := preAcceptS.(*epaxosprotobuf.PreAccept)
			r.handlePreAccept(preAccept)
			break

		case acceptS := <-r.acceptChan:
			accept := acceptS.(*epaxosprotobuf.Accept)
			r.handleAccept(accept)
			break

		case commitS := <-r.commitChan:
			commit := commitS.(*epaxosprotobuf.Commit)
			r.handleCommit(commit)
			break

		case commitS := <-r.commitShortChan:
			commit := commitS.(*epaxosprotobuf.CommitShort)
		...
		case preAcceptReplyS := <-r.preAcceptReplyChan:
			preAcceptReply := preAcceptReplyS.(*epaxosprotobuf.PreAcceptReply)
			r.handlePreAcceptReply(preAcceptReply)
			break

		case preAcceptOKS := <-r.preAcceptOKChan:
			preAcceptOK := preAcceptOKS.(*epaxosprotobuf.PreAcceptOK)
			r.handlePreAcceptOK(preAcceptOK)
			break

		case acceptReplyS := <-r.acceptReplyChan:
			acceptReply := acceptReplyS.(*epaxosprotobuf.AcceptReply)
...
func (r *Replica) replyPreAccept(replicaId int32, reply *epaxosprotobuf.PreAcceptReply) {
	r.SendMsg(replicaId, r.preAcceptReplyRPC, reply)
}

func (r *Replica) replyAccept(replicaId int32, reply *epaxosprotobuf.AcceptReply) {
	r.SendMsg(replicaId, r.acceptReplyRPC, reply)
}

var pa epaxosprotobuf.PreAccept

func (r *Replica) bcastPreAccept(replica int32, instance int32, ballot int32, cmds []*epaxosprotobuf.Command, seq int32, deps []int32) {
	pa.LeaderId = r.Id
	pa.Replica = replica
...
var ec epaxosprotobuf.Commit
var ecs epaxosprotobuf.CommitShort

func (r *Replica) bcastCommit(replica int32, instance int32, cmds []*epaxosprotobuf.Command, seq int32, deps []int32) {
	ec.LeaderId = r.Id
	ec.Replica = replica
...
func (r *Replica) handlePropose(propose *genericsmr.Propose) {
	...
	if batchSize == 1 {
		//log.Println("batchSize==1, ignore single proposal")
	...
	//log.Printf("Starting instance %d.%d\n", r.Id, instNo)
...
func (r *Replica) handlePreAccept(preAccept *epaxosprotobuf.PreAccept) {
	...
	if changed || uncommittedDeps || preAccept.Replica != preAccept.LeaderId || !isInitialBallot(preAccept.Ballot) {
		...
		pok := &epaxosprotobuf.PreAcceptOK{preAccept.Instance}
		r.SendMsg(preAccept.LeaderId, r.preAcceptOKRPC, pok)
	}
}

func (r *Replica) handlePreAcceptReply(pareply *epaxosprotobuf.PreAcceptReply) {
	...
	if pareply.Ok == epaxosprotobuf.NONE {
...
func (r *Replica) handlePreAcceptOK(pareply *epaxosprotobuf.PreAcceptOK) {
...
func (r *Replica) handleAccept(accept *epaxosprotobuf.Accept) {
...
func (r *Replica) handleAcceptReply(areply *epaxosprotobuf.AcceptReply) {
