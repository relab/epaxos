...
func main() {
	...
	for i := 0; i < N; i++ {
		host, port, err := net.SplitHostPort(rlReply.ReplicaList[i])
		portAsInt, err := strconv.Atoi(port)
		if err != nil {
			panic(err)
		}
		addr := fmt.Sprintf("%s:%d", host, portAsInt+10)
		servers[i], err = net.Dial("tcp", addr)
		if err != nil {
	...
	args := &genericsmrprotobuf.Propose{id, &epaxosprotobuf.Command{epaxosprotobuf.PUT, 0, 0}, 0}
	...
	for j := 0; j < *rounds; j++ {
		...
		for i := 0; i < n+*eps; i++ {
			...
			if !*fast {
				...
				writers[leader].WriteByte(byte(args.Size()))
